...
# flags = tf.app.flags
# FLAGS = flags.FLAGS
# flags.DEFINE_string('A_readmodels', os.path.abspath('./TrainningResource/Alphabet/Alphabet_model/model.ckpt'), 'File name of model data')
...
def inference(images_placeholder, keep_prob):
    ...
    with tf.name_scope('A_fc1') as scope:
    ...
    with tf.name_scope('A_fc2') as scope:
        W_fc2 = weight_variable([1024, NUM_CLASSES])
        b_fc2 = bias_variable([NUM_CLASSES])

    with tf.name_scope('A_softmax') as scope:
...
def main(ImagePath):
    ...
    Dict = get_particular_variables('A')
    saver = tf.train.Saver(Dict)
    sess.run(tf.global_variables_initializer())
    saver.restore(sess,os.path.abspath('./TrainningResource/Alphabet/Alphabet_model/model.ckpt'))
    ret =[]
